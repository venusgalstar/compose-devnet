...
Find below a list of `composed` commands added with the `x/incentives` module. You can obtain the full list by using the `composed -h` command.
...
```go
composed query incentives incentives [flags]
```
...
```go
composed query incentives incentive CONTRACT_ADDRESS [flags]
```
...
```bash
composed query incentives gas-meters CONTRACT_ADDRESS [flags]
```
...
```go
composed query incentives gas-meter CONTRACT_ADDRESS PARTICIPANT_ADDRESS [flags]
```
...
```bash
composed query incentives params [flags]
```
...
```bash
composed tx gov submit-proposal register-incentive CONTRACT_ADDRESS ALLOCATION EPOCHS [flags]
```
...
```bash
composed tx gov submit-proposal cancel-incentive CONTRACT_ADDRESS [flags]
```
...
```bash
composed tx gov submit-proposal param-change PROPOSAL_FILE [flags]
```
