...
Find below a list of `composed` commands added with the `x/vesting` module. You can obtain the full list by using the `composed -h` command.
...
```go
composed add-genesis-account ADDRESS_OR_KEY_NAME COIN... [flags]
```
...
```go
composed query vesting balances ADDRESS [flags]
```
...
```go
composed tx vesting create-clawback-vesting-account TO_ADDRESS [flags]
```
...
```go
composed tx vesting clawback ADDRESS [flags]
```
...
```go
composed tx vesting update-vesting-funder VESTING_ACCOUNT_ADDRESS NEW_FUNDER_ADDRESS --from=FUNDER_ADDRESS [flags]
```
