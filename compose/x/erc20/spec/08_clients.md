...
Find below a list of  `composed` commands added with the  `x/erc20` module. You can obtain the full list by using the `composed -h` command. A CLI command can look like this:

```bash
composed query erc20 params
```
...
```bash
composed tx gov submit-proposal register-coin METADATA_FILE [flags]
```
...
```bash
composed tx gov submit-proposal register-erc20 ERC20_ADDRESS... [flags]
```
...
```bash
composed tx gov submit-proposal toggle-token-conversion TOKEN [flags]
```
