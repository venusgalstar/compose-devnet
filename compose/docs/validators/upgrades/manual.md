...
Before upgrading the Evmos version. Stop your instance of `composed` using `Ctrl/Cmd+C`.
...
::: tip
If the software version does not match, then please check your `$PATH` to ensure the correct `composed` is running.
:::
...
```bash
rm $HOME/.composed/config/addrbook.json
