...
```bash
composed init $moniker --chain-id $CHAIN_ID
```

Move genesis file to .composed/config folder

```bash
mv $HOME/genesis.json ~/.composed/config/
```
...
```bash
sed -i -e "s%^moniker *=.*%moniker = \"$moniker\"%; " $HOME/.composed/config/config.toml
sed -i -e "s%^indexer *=.*%indexer = \"null\"%; " $HOME/.composed/config/config.toml
sed -i -e "s%^persistent_peers *=.*%persistent_peers = \"$PEER\"%; " $HOME/.composed/config/config.toml
```
...
s|^(seeds[[:space:]]+=[[:space:]]+).*$|\1\"\"|" ~/.composed/config/config.toml
```
...
```bash
echo "[Unit]
Description=composed Node
After=network.target
#
[Service]
User=$USER
Type=simple
ExecStart=$(which composed) start
Restart=on-failure
LimitNOFILE=65535
#
[Install]
WantedBy=multi-user.target" > $HOME/composed.service; sudo mv $HOME/composed.service /etc/systemd/system/
```

```bash
sudo systemctl enable composed.service && sudo systemctl daemon-reload
```

### Run composed

```bash
sytemctl start composed
```

### Check logs

```bash
journalctl -u composed -f
```
...
```bash
sed -i.bak -E "s|^(enable[[:space:]]+=[[:space:]]+).*$|\1false|" $HOME/.composed/config/config.toml
```
