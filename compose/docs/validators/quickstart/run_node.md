...
```bash
composed start --json-rpc.enable=true --json-rpc.api="eth,web3,net"
```

## Key Management

To run a node with the same key every time: replace `composed keys add $KEY` in `./local_node.sh` with:

```bash
echo "your mnemonic here" | composed keys add $KEY --recover
```
...
```bash
composed config keyring-backend file
```
...
```bash
composed start --evm.tracer $TRACER --tracestore $TRACESTORE --pruning=nothing $TRACE --log_level $LOGLEVEL --minimum-gas-prices=0.0001pose --json-rpc.api eth,txpool,personal,net,debug,web3
```
...
::: danger
If you are running a **validator node**, always be careful when doing `composed unsafe-reset-all`. You should never use this command if you are not switching `chain-id`.
:::
