...
calling the `composed testnet` command. This outputs a handful of files in the
`./build` directory:
...
Each `./build/nodeN` directory is mounted to the `/composed` directory in each container.
...
```bash
# node 0: daemon logs
docker exec composednode0 tail composed.log
...
To interact with `composed` and start querying state or creating txs, you use the
`composed` directory of any given node as your `home`, for example:

```bash
composed keys list --home ./build/node0/composed
```
...
::: tip
**Note**: Each node's seed is located at `./build/nodeN/composed/key_seed.json` and can be restored to the CLI using the `composed keys add --restore` command
:::
